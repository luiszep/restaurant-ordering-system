...
const AddItemsToCategoryModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      setSelectedItemIds([]); // reset selection when modal opens
    }
  }, [isOpen]);
  ...
  const handleAddSelectedItems = () => {
    const itemsToAdd = MOCK_ITEMS.filter((item) =>
    ...
    <div className="fixed inset-0 bg-white/70 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
        ...
        <ul className="divide-y border rounded mb-4 max-h-[300px] overflow-y-auto">
          {availableItems.map((item) => (
            <li key={item.id} className="flex justify-between items-center px-4 py-3">
              ...
              >
                {selectedItemIds.includes(item.id) ? 'Added' : 'Add'}
              </button>
            </li>
          ))}
        </ul>
        ...
        <div className="text-sm text-gray-700 font-medium mt-1">
          ...
          item{selectedItemIds.length !== 1 && 's'} to the <span className="font-bold">“{categoryName}”</span> category.
        </div>
